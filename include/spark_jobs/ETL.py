...
from pyspark.sql.functions import max as spark_max, row_number, col, create_map, lit, when, lower, regexp_replace, to_date, to_timestamp, dayofmonth, month, year, dayofweek, date_format, weekofyear, quarter, coalesce

# Initialize Spark Session
spark = SparkSession.builder \
    .appName("ETL_Process") \
    .config("spark.jars", "/usr/local/airflow/postgresqljdbc.jar") \
    .getOrCreate()
...
file2 = r"/usr/local/airflow/include/Transactions5.csv"
...
transactions_full_df = df_transactionsdb.join(df_customers, on="Customer ID", how="inner") \
    ...
    .join(date_dwh, df_transactionsdb["Transaction Date"] == date_dwh["date"], "left") \
    .select(
        df_transactionsdb["Transaction ID"],
        date_dwh["date_pk_bk"],
        df_transactionsdb["Store ID"],
        df_transactionsdb["Employee ID"].alias("employeeid_pk_bk"),
        df_transactionsdb["Customer ID"].alias("customerid_pk_bk"),
        df_transactionsdb["Discount ID"].alias("discountid_pk_bk"),
        df_transactionsdb["Invoice ID"].alias("invoiceid"),
        df_transactionsdb["Transaction Type"].alias("transactiontype"),
        df_transactionsdb["Payment Method"].alias("paymentmethod"),
        df_transactionsdb["Invoice Total"].alias("invoicetotal")
    )

transaction_lines_cat = df_transactionlines.join(df_productattribute, "Product ID", "inner") \
    .join(df_products, "Product ID", "left") \
...
transactions_dwh = transaction_lines_cat.join(transactions_full_df, on="Transaction ID", how="inner").select(
...
pandas_df.to_csv(r"/usr/local/airflow/include/data/date_dim.csv", index=False)

# Save Customers DWH
pandas_df = customers_dwh.toPandas()
pandas_df.to_csv(r"/usr/local/airflow/include/data/customers_dim.csv", index=False)

# Save Discounts DWH
pandas_df = discounts_dwh.toPandas()
pandas_df.to_csv(r"/usr/local/airflow/include/data/discounts_dim.csv", index=False)

# Save Products DWH
pandas_df = products_dwh.toPandas()
pandas_df.to_csv(r"/usr/local/airflow/include/data/products_dim.csv", index=False)

# Save Stores DWH
pandas_df = stores_dwh.toPandas()
pandas_df.to_csv(r"/usr/local/airflow/include/data/stores_dim.csv", index=False)

# Save Employees DWH
pandas_df = employees_dwh.toPandas()
pandas_df.to_csv(r"/usr/local/airflow/include/data/employees_dim.csv", index=False)

# Save Transactions Fact Table
pandas_df = transactions_dwh.toPandas()
pandas_df.to_csv(r"/usr/local/airflow/include/data/transactions_fact.csv", index=False)
